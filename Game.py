...
class Tribble(Button):
                    ...
                    if len(alleles[0]) == 0:
                        ...
                        match alleles[0][0]:
                            case "Y":
                                self.sex = "male"
                case "fur_color":
                    if len(alleles[0]) == 2 and (alleles[0][0]) != (alleles[0][1]) or (len(alleles[1]) == 2 and (alleles[1][0]) != (alleles[1][1])):
                        mixer = []
                        if len(alleles[0]) == 2:
                            alleles = alleles[0]
    ...
    def update(self):
        ...
        elif not self.rect.collidelistall(obstacles) == [] or self.rect.collidelistall(tribblerects) and self.collisions:
            self.collisions = False
            self.goal = onedge(field_rect)
            self.movetowards(self.goal)
...
Tribble_list = {
    "Joe": Tribble({"sex": ["x", "Y"], "eye_color": ["B", "u"], "spottiness": ["S", "p"], "fur_color": ["R", "U"], "spot_color": ["R", "U"], "fertility": ["F", "f"]}, "Joe"),
    "Eve": Tribble({"sex": ["x", "x"], "eye_color": ["u", "u"], "spottiness": ["p", "p"], "fur_color": ["R", "w"], "spot_color": ["b", "b"], "fertility": ["F", "F"]}, "Eve")
}
